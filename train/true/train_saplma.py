...
class Model():
    ...
    def run(self, optim):
        ...
        data_types = ["animals", "cities", "companies", "elements", "facts", "inventions"]

        train_number = self.args.train_number
        if train_number < 0 or train_number >= len(data_types):
            raise ValueError("train_number must be between 0 and {}".format(len(data_types) - 1))

        train_data_type = data_types[train_number]
        eval_data_types = [dt for i, dt in enumerate(data_types) if i != train_number]

        data = get_data(self.args.data_path, train_data_type)
        print("Train data loaded for:", train_data_type)
        ...
        eval_data = {}
        for dt in eval_data_types:
            eval_data[dt] = get_data(self.args.data_path, dt)
            print("Eval data loaded for:", dt)
